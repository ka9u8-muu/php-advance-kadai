...
if(isset($_POST['submit'])){
    try{
        ...
        // 動的に変わる値をプレースホルダに置き換えたUPDATE文を予め用意する
        $sql_update = '
        UPDATE books
        ...
        // bindValue()メソッドを使って実際の値をプレースホルダに割り当てる
...
if(isset($_GET['id'])){
    try{
        ...
        // 1つのレコードのみを取得したい場合、fetch()メソッドを使えばカラム名がキーになった配列を取得できる

        // idパラメータの値と同じidのデータが存在しない時、エラーメッセージを表示して処理を終了する
        // fetch()メソッドは実行結果が取得できなかった場合にFALSEを返す
        if($book === FALSE){
            exit('idパラメータの値が不正です。');
        }
        // vendorsテーブルからvendor_codeカラムのデータを取得するためのSQL文を変数$sql_select_vendor_codeに代入
        ...
        // PDO::FETCH_COLUMNは1つのカラムの値を配列で取得する設定

    }catch(PDOException $e){
        exit($e->getMessage());
    }
}else{
    // idパラメータが存在しない時、エラーメッセージを表示して処理を停止
    exit('idパラメータが存在しません。');
}
?>
                        ...
                        <div>
                            ...
                            <select id="genre_code" name="genre_code" required>
                                ...
                                foreach($genre_codes as $genre_code){
                                    // 変数$vendor_codeが商品の仕入れ先コードの値と一致した時、selected属性をつけて初期値にする
